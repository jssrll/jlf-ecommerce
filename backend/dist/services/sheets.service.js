...
export async function loginUser(phone, password) {
    ...
    await appendToSheet(SHEETS.LOGIN_LOGS, ['Timestamp', 'Account ID', 'Full Name', 'Phone', 'Password', 'Status'], [new Date().toISOString(), user.accountId, user.name, user.phone, user.password, 'Success']);
    return { success: true, user };
}
export async function registerUser(name, phone, password, accountId) {
    const existing = await getUserByPhone(phone);
    if (existing) {
        return { success: false, message: 'Phone number already registered' };
    }
    await appendToSheet(SHEETS.ACCOUNTS, ['Timestamp', 'Account ID', 'Name', 'Phone', 'Password', 'Balance'], [new Date().toISOString(), accountId, name, phone, password, 0]);
    await appendToSheet(SHEETS.LOYALTY_CARDS, ['Account ID', 'Full Name', 'Phone', 'Marks', 'Total Earned', 'Last Scan Date', 'Created At'], [accountId, name, phone, 0, 0, '', new Date().toISOString()]);
    clearCache('users');
...
// ORDER SERVICE
// ========================================
export async function placeOrder(user, orderList, totalPrice) {
    const balanceResult = await updateBalance(user.phone, totalPrice, 'deduct');
    if (!balanceResult.success) {
        return balanceResult;
    }
    await appendToSheet(SHEETS.ORDERS, ['Timestamp', 'Full Name', 'Account ID', 'Phone', 'Order List', 'Total Price', 'Status'], [new Date().toISOString(), user.name, user.accountId, user.phone, orderList, totalPrice, 'Pending']);
...
export async function updateOrderStatus(timestamp, phone, status) {
    const data = await getSheetData(SHEETS.ORDERS);
...
export async function submitRecharge(data) {
    await appendToSheet(SHEETS.RECHARGE_REQUESTS, ['Timestamp', 'Account ID', 'Full Name', 'Phone', 'Method', 'Amount', 'Reference', 'Status'], [new Date().toISOString(), data.accountId, data.fullName, data.phone, data.method, data.amount, data.reference, 'Pending']);
    return { success: true, message: 'Recharge request submitted' };
}
export async function getUserRecharges(phone) {
...
export async function updateRechargeStatus(timestamp, phone, status) {
    const data = await getSheetData(SHEETS.RECHARGE_REQUESTS);
    for (let i = 1; i < data.length; i++) {
        if (data[i][0] === timestamp && data[i][3] === phone) {
...
export async function submitWithdrawal(data) {
    await appendToSheet(SHEETS.WITHDRAWAL_REQUESTS, ['Timestamp', 'Account ID', 'Full Name', 'Phone', 'Method', 'Amount', 'Receiver Name', 'Receiver Number', 'Status'], [new Date().toISOString(), data.accountId, data.fullName, data.phone, data.method, data.amount, data.receiverName, data.receiverNumber, 'Pending']);
    return { success: true, message: 'Withdrawal request submitted' };
}
export async function getUserWithdrawals(phone) {
...
export async function updateWithdrawalStatus(timestamp, phone, status) {
    const data = await getSheetData(SHEETS.WITHDRAWAL_REQUESTS);
    for (let i = 1; i < data.length; i++) {
        if (data[i][0] === timestamp && data[i][3] === phone) {
...
export async function redeemCode(code, user) {
    ...
    await appendToSheet(SHEETS.CODE_REDEMPTION, ['Timestamp', 'Account ID', 'Full Name', 'Phone', 'Code Input', 'Reward', 'Type'], [new Date().toISOString(), user.accountId, user.name, user.phone, code, `${reward} peso credit`, 'one-time']);
    return {
        success: true,
...
export async function getUserRedemptions(phone) {
...
export async function addPromoCode(code, reward, expiryDate, description) {
    ...
    await appendToSheet(SHEETS.PROMO_CODES, ['Code', 'Reward (₱)', 'Status', 'Redeemed By (Account ID)', 'Redeemed By (Phone)', 'Redeemed At', 'Expiry Date', 'Description'], [code, reward, 'unused', '', '', '', expiryDate, description]);
...
export async function addCreditInvestment(data) {
    await appendToSheet(SHEETS.CREDIT_INVESTMENTS, ['Timestamp', 'Account ID', 'Full Name', 'Phone', 'Investment Type', 'Amount (₱)', 'Expected Return (₱)', 'Status', 'Maturity Date', 'Duration Days'], [new Date().toISOString(), data.accountId, data.fullName, data.phone, data.investmentType, data.amount, data.expectedReturn, 'Active', data.maturityDate, data.durationDays]);
    return { success: true, message: 'Investment recorded' };
}
export async function getUserCreditInvestments(phone) {
...
// LOYALTY SERVICE
// ========================================
export async function getUserLoyalty(phone) {
...
export async function checkUserLoyalty(phone) {
    const data = await getSheetData(SHEETS.LOYALTY_CARDS);
    const user = data.find(row => row[2] === phone);
    return { exists: !!user, marks: user ? parseInt(user[3]) || 0 : 0 };
}
export async function createUserLoyalty(accountId, fullName, phone) {
    ...
    await appendToSheet(SHEETS.LOYALTY_CARDS, ['Account ID', 'Full Name', 'Phone', 'Marks', 'Total Earned', 'Last Scan Date', 'Created At'], [accountId, fullName, phone, 0, 0, '', new Date().toISOString()]);
    return { success: true, message: 'Loyalty record created', marks: 0 };
}
export async function addLoyaltyScan(accountId, phone, scannedBy) {
    const data = await getSheetData(SHEETS.LOYALTY_CARDS);
    ...
    await appendToSheet(SHEETS.LOYALTY_SCANS, ['Timestamp', 'Account ID', 'Full Name', 'Phone', 'Scanned By', 'Marks Before', 'Marks After', 'Reward Claimed'], [new Date().toISOString(), accountId, user[1], user[2], scannedBy, currentMarks, newMarks, rewardClaimed ? 'Yes (₱99)' : 'No']);
    return {
        success: true,
...
export async function addAnnouncement(data) {
    await appendToSheet(SHEETS.ANNOUNCEMENTS, ['Timestamp', 'Date', 'Header', 'Content', 'Type', 'Priority', 'Icon', 'Link', 'LinkText', 'ExpiryDate', 'PublishedBy', 'Status'], [new Date().toISOString(), new Date().toLocaleDateString(), data.header, data.content, data.type, data.priority, data.icon, data.link, data.linkText, data.expiryDate, 'Admin', 'active']);
    return { success: true, message: 'Announcement published!' };
}
export async function updateAnnouncementStatus(timestamp, status) {
    const data = await getSheetData(SHEETS.ANNOUNCEMENTS);
    for (let i = 1; i < data.length; i++) {
...
export async function addBugReport(data) {
    await appendToSheet(SHEETS.BUG_REPORTS, ['Timestamp', 'Date', 'Account ID', 'Full Name', 'Phone', 'Bug Report', 'Status', 'Device Info'], [new Date().toISOString(), data.date, data.accountId, data.fullName, data.phone, data.bugReport, 'pending', data.deviceInfo]);
...
export async function updateBugReportStatus(timestamp, status) {
    const data = await getSheetData(SHEETS.BUG_REPORTS);
    for (let i = 1; i < data.length; i++) {
        if (data[i][0] === timestamp) {
