...
export async function login(req, res) {
    try {
        ...
        const token = jwt.sign({ phone: result.user?.phone, accountId: result.user?.accountId, name: result.user?.name, role: 'user' }, process.env.JWT_SECRET, { expiresIn: '24h' });
        res.json({
            success: true,
            token,
            user: {
...
export async function register(req, res) {
    try {
        ...
        const token = jwt.sign({ phone: data.phone, accountId: data.accountId, name: data.name, role: 'user' }, process.env.JWT_SECRET, { expiresIn: '24h' });
